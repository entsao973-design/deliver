...
test("admin user management has account and permission assignment panels", () => {
  ...
  assert.match(userFormControlBlock, /height:\s*auto;/);
  assert.match(userFormControlBlock, /min-height:\s*0;/);
  assert.doesNotMatch(userFormControlBlock, /height:\s*24px|min-height:\s*24px/);
  const userFormButtonBlock = cssBlockAfter(css, ".user-form button {");
  assert.match(userFormButtonBlock, /height:\s*auto;/);
  assert.match(userFormButtonBlock, /min-height:\s*0;/);
