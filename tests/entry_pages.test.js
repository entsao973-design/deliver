...
test("admin delivery filter labels align with their control text", () => {
  ...
  assert.match(css, /\.delivery-filter-grid > label:nth-child\(-n \+ 4\) > span\s*\{\s*padding-inline-start:\s*12px;\s*\}/);
});
...
test("admin delivery sort buttons toggle direction and query restores original order", () => {
  ...
  assert.match(css, /\.delivery-list-toolbar\s*\{[\s\S]*display:\s*grid;[\s\S]*grid-template-columns:\s*minmax\(180px,\s*4fr\) minmax\(160px,\s*3fr\) minmax\(120px,\s*2fr\) minmax\(76px,\s*1fr\) minmax\(100px,\s*2\.1fr\);[\s\S]*min-height:\s*34px;/);
