...
Android 手機瀏覽器可用的配送存證平台 MVP。系統會讀取每日配送 Excel，司機以帳號、密碼、車號登入後，只看到該車號的配送單據。
...
- `配送紀錄維護`：位於封存照片旁，可依開始日期與結束日期永久清除歷史資料。
...
- 日期區間包含開始日期與結束日期。
- 永久清除區間內全部配送紀錄，包括未達交、已達交與刪除區資料。
- 同時清除對應日期的已達交照片資料夾與已封存 ZIP。
- 執行前會顯示不可恢復警告；確認執行後無法復原。
